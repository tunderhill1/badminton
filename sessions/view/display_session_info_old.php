  ...
  /* Display table contents */
	for($i=0; $i<$num_rows; $i++){
		...
		if($session->userlevel >= LEVEL_MEMBER){
			...
			if($dayofweek == 4 && !$database->isUserAlreadyBookedTraining($session->username) && !$database->isUserBookedToSession($session->username,$session_id) && $database->remainingSessionPlaces($session_id)>0){
			...
			} else if ($dayofweek != 4 && !$database->isUserAlreadyBookedNonTraining($session->username) && !$database->isUserBookedToSession($session->username,$session_id) && $database->remainingSessionPlaces($session_id)>0){
				$opbuttons .= "<form action=\"".SITE_URL."/include/process.php\" method=\"POST\">
				<input type=\"hidden\" name=\"sessionid\" value=\"$session_id\">";
				$opbuttons .= "<input type=\"submit\" name=\"addbooking\" value=\"Book\" class=\"link\"></form>";
			}
