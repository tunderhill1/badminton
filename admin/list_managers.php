...
function displayAllMembers(){
	global $database;
	$q = "SELECT * FROM ".TBL_USERS." WHERE usr_lvl>=".LEVEL_MANAGER." ORDER BY lastname";
...
if($session->userlevel >= LEVEL_COMMITTEE){
	...
	<h1>Session managers list</h1>
	<p>This lists according to the website's "userlevel" feature, so reflects only what is stored in the database.</p>
